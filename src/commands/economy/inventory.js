...
function describeItem(item) {
  ...
  if (item.type === 'vapejuice') {
    return `**Vape Juice** - charges \`${item.charges || 0}\`, status \`${item.status}\``;
  }

  if (item.type === 'cigarette') {
    return `**Cigarette Pack** - sticks \`${item.sticks || 0}\`, smoked \`${item.smoked || 0}\`, status \`${item.status}\``;
  }

  if (item.type === 'lighter') {
    return `**Lighter** - charges \`${item.charges || 0}\`, status \`${item.status}\``;
  }
