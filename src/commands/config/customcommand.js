...
module.exports = {
  ...
  async execute({ message, args }) {
    ...
    if (sub === 'add' || sub === 'set') {
      ...
      const limit = access?.limits?.customCommands || 30;
      ...
      if (existing && !existing.some((row) => row.name === name) && existing.length >= limit) {
        return respond.reply(
          ...
          access?.hasServerPremiumBase
            ? `This server already used all ${limit} custom command slots.`
            : 'Free servers can save up to 30 custom commands. Server premium raises that to 200.'
        );
      }
