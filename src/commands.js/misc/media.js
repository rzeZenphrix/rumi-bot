...
const EDIT_OPERATIONS = new Set([
  'blur', 'brightness', 'caption', 'contrast', 'deepfry', 'flip', 'gif',
  'glitch', 'grayscale', 'invert', 'jail-bars', 'pixelate', 'resize',
  'rotate', 'vaporwave', 'wasted'
]);
...
function looksLikeMediaUrl(url) {
  ...
  return clean.endsWith('.png') || clean.endsWith('.jpg') || clean.endsWith('.jpeg') ||
    ...
    clean.endsWith('.webm') || clean.includes('media.tenor.com');
}
...
function parseEditOptions(operation, args) {
  ...
  if (operation === 'rotate') {
    return { degrees: Number(args[0] || 90) || 90 };
  }
  ...
  if (operation === 'pixelate') {
    return { factor: Number(args[0] || 14) || 14 };
  }
...
async function resolveTenorWithApi(url) {
  ...
  const picked = formats.gif?.url || formats.mediumgif?.url || formats.tinygif?.url || formats.webp?.url || formats.mp4?.url;
  if (!picked) return null;

  return { url: picked, name: filenameFromUrl(picked, 'tenor.gif'), contentType: contentTypeFromUrl(picked) || 'image/gif' };
}
...
async function resolveTenorWithHtml(url) {
  ...
  for (const pattern of patterns) {
    ...
    if (match) {
      return { url: match, name: filenameFromUrl(match, 'tenor.gif'), contentType: contentTypeFromUrl(match) };
    }
  }
...
async function resolveMediaUrl(url) {
  ...
  if (isTenorUrl(url)) {
    if (String(url).includes('media.tenor.com')) {
      return { url, name: filenameFromUrl(url, 'tenor.gif'), contentType: contentTypeFromUrl(url) || 'image/gif' };
    }
    return (await resolveTenorWithApi(url)) || (await resolveTenorWithHtml(url));
  }

  if (!looksLikeMediaUrl(url)) return null;
  return { url, name: filenameFromUrl(url), contentType: contentTypeFromUrl(url) };
}
...
async function getMediaSourceFromMessage(target) {
  ...
  const embedMedia = target.embeds?.find?.((embed) => embed.image?.url || embed.thumbnail?.url || embed.video?.url || embed.url);
  const embedUrl = embedMedia?.image?.url || embedMedia?.thumbnail?.url || embedMedia?.video?.url || embedMedia?.url || null;
  if (embedUrl) {
...
module.exports = {
  ...
  subcommands: [
    { name: 'caption', aliases: ['cap'], description: 'Caption an image, GIF, video, or Tenor link.', usage: 'media caption [url] <text>', examples: ['media caption meow', 'media caption --position=bottom --font=bold cute'] },
    { name: 'gif', aliases: ['exportgif'], description: 'Fetch and return the original GIF or media file.', usage: 'media gif [url]', examples: ['media gif', 'media gif https://tenor.com/view/example'] },
  ...
  async execute({ message, args }) {
    ...
    if (!EDIT_OPERATIONS.has(sub)) {
      return respond.reply(message, 'info', 'Use `media <caption|gif|blur|brightness|contrast|deepfry|flip|glitch|grayscale|invert|jail-bars|pixelate|resize|rotate|vaporwave|wasted> ...`.');
    }
    ...
    if (sub === 'caption') {
      ...
      const output = await makeCaptionedImage(input, caption, source.name || 'captioned.gif', source.contentType || '', parsed.options);
    ...
    const output = await makeEditedImage(input, source.name || `${sub}.png`, sub, parseEditOptions(sub, args));
    return sendMedia(message, output);
  }
};
