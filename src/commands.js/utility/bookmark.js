...
module.exports = {
  ...
  async execute({ message, args }) {
    ...
    if (sub === 'add') {
      ...
      if (saved && saved.length >= (access?.limits?.bookmarkSlots || 5)) {
        return respond.reply(
          message,
          'bad',
          access?.hasUserPremium
            ? 'You already used all 20 bookmark slots.'
            : 'Free users can save up to 5 bookmarks. User premium raises that to 20.'
        );
      }
